...
bool EnterExtendedSession(int udsHandle, const DelaySettings& delays)//вход в расширенноую сессию, можно быдл универсальную сессию
{
...
void TryWriteInSession(int udsHandle, const DidRefEntry& entry, char* outStatus, bool useExtended)
{
    ...
    if (writeStatus == 0)
    {
        strcpy(outStatus, useExtended ? "WRITE_OK_EXT" : "WRITE_OK_def");
    }
    else
    {
        strcpy(outStatus, useExtended ? "WRITE_FAIL_EXT" : "WRITE_FAIL_DEF");
    }
...
void WriteReportHeader(native_int reportHandle)
{
    const char* header[1] = {
        ...
        "RANGE_STATUS\tMIN_HEX\tMAX_HEX\tOUT_OF_RANGE_TEST"
    };
    app.write_text_file_line_string_array(reportHandle, header, 1);
}
//запись полученных значений из ex, def session
...
void WriteTotalStats(native_int reportHandle, const CheckStats& stats)//запись статистики
{
    char totalLine[200];
...
//чистка таблицы подумать в DidRefC
        DIDResult res;
        res.entry = entry;
    ...
    app.terminate_application();//нет проверки на усппешное создание репорта 
}
